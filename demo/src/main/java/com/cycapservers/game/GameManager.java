...
public class GameManager {
	...
	// create player afk list that has the players time out after 30 seconds and
	// get deleted
	...
	public void getMessage(WebSocketSession session, String message) throws IOException {
		...
		if (arr[0].equals("input")) {
			...
			for (GameState gs : games) {
				found = gs.findIncomingPlayer(arr[1], session); // TODO: the
																// client should
																// not send
																// role, this
																// should be
			...
			if (!found) {
				games.get(0).playerJoin(arr[1], session, "recruit"); // the
																		// player
																		// can
																		// only
																		// play
	...
	/**
	 * Gives a list of all of the players in the lobby that is represented by
	 * the id.
	 * 
	 * @param session
	...
	private void GivePlayerList(WebSocketSession session, String id) throws IOException {
		...
		if (game != -1) {
			...
			// session.sendMessage(new TextMessage("player:"+
			// lobby.get(game).getPlayer(i)));
	...
	/*
	 * public void removePlayer(String Id){ for(int i = 0; i <
	 * this.player.size(); i++){ if(this.player.get(i).getName().equals(Id)){
	 * this.player.remove(i); break; } } }
	 * 
	 * public void getAFK(){ long time = System.currentTimeMillis(); for(int i =
	 * 0; i < this.player.size(); i++){ if((time - this.player.get(i).getTime())
	 * > 5000){ this.removedPlayer.add(this.player.get(i).getName());
	 * this.player.remove(i); this.afkPlayers = true; } } }
	 * 
	 * public long GetPlayerTime(int i){ return this.player.get(i).getTime(); }
	 * 
	 * 
	 * public int getPlayers(){ return this.player.size(); }
	 * 
	 * public String playerString(int i){ return this.player.get(i).toString();
	 * }
	 * 
