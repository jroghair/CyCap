...
public final class Utils {
	...
	public final static SpeedPotion SPEED_POTION = new SpeedPotion(0, 0, GRID_LENGTH, GRID_LENGTH, 0, 1.0,
			"speed_pot_template");
	public final static HealthPack HEALTH_PACK = new HealthPack(0, 0, GRID_LENGTH, GRID_LENGTH, 0, 1.0,
			"health_pack_template");
	...
	public static boolean isColliding(Entity ent_1, Entity ent_2) {
		...
		boolean y_collision = isBetween(ent_1.y - (ent_1.drawHeight / 2), ent_2.y - (ent_2.drawHeight / 2),
				ent_2.y + (ent_2.drawHeight / 2))
				|| isBetween(ent_1.y + (ent_1.drawHeight / 2), ent_2.y - (ent_2.drawHeight / 2),
						ent_2.y + (ent_2.drawHeight / 2))
				|| isBetween(ent_1.y, ent_2.y - (ent_2.drawHeight / 2), ent_2.y + (ent_2.drawHeight / 2));

		if (isBetween(ent_1.x - (ent_1.drawWidth / 2), ent_2.x - (ent_2.drawWidth / 2), ent_2.x + (ent_2.drawWidth / 2))
				&& y_collision) {
			return true;
		} else if (isBetween(ent_1.x + (ent_1.drawWidth / 2), ent_2.x - (ent_2.drawWidth / 2),
				ent_2.x + (ent_2.drawWidth / 2)) && y_collision) {
			return true;
		} else if (isBetween(ent_1.x, ent_2.x - (ent_2.drawWidth / 2), ent_2.x + (ent_2.drawWidth / 2))
				&& y_collision) {
			return true;
		} else {
			return false;
		...
		/*
		 * This was the older method of collision detection. it is simpler and
		 * could still be used for more basic detection if (isBetween(ent_1.x,
		 * (ent_2.x - (ent_2.dWidth/2)), (ent_2.x + (ent_2.dWidth/2))) &&
		 * isBetween(ent_1.y, (ent_2.y - (ent_2.dHeight/2)), (ent_2.y +
		 * (ent_2.dHeight/2)))){ return true; } else{ return false; }
		 */
	}
	 ...
	 * 
	 * @param p
	 *            The player which we are setting the role/class data
	 */
	...
	public static ArrayList<ArrayList<mapNode>> generate_node_array(GameState g) { // passing
																					// GameState
																					// so
																					// it
																					// doesn't
		...
		short index_i = 0;
		short index_j = 0;
		ArrayList<ArrayList<mapNode>> map = new ArrayList<ArrayList<mapNode>>(); // 2d
																					// map
																					// of
																					// nodes
		for (int i = 0; i < (GRID_LENGTH * g.mapGridWidth); i += Utils.AI_NODE_PIXEL_DISTANCE) {
			...
			for (int j = 0; j < (GRID_LENGTH * g.mapGridHeight); j += Utils.AI_NODE_PIXEL_DISTANCE) {
				Entity test_player_ent = new Entity(0, 0, i, j, GRID_LENGTH, GRID_LENGTH, 0, 1, "temp");// used
																										// for
																										// traversing
				...
				for (int t = 0; t < g.walls.size(); t++) {
					if (isColliding(test_player_ent, g.walls.get(t))) {
						traversable = false;
						break; // colliding with one wall is enough to know this
								// isn't a possible space
					}
				}
	...
	public static ArrayList<mapNode> get_neighbors(GameState g, mapNode node, ArrayList<mapNode> closed_list,
