...
let masks = [];
var player;
var shot_this_turn = false; //for knowing whether or not to add a bullet to data packet
let grid_length = (canvas.width / 30); //the grid map we are using for now is 30x20
var player_speed = 2,
  bullet_speed = 5; //pixels. eventually we will want this to be based on grid_length/seconds
var mouseX, mouseY;
...
function create_data_packet() {
  var check = ('{' + player.x + ',' + player.y + '}');
  if (shot_this_turn) {
    check += ('{' + bullets[bullets.length - 1].x_ratio + ',' +
      (bullets[bullets.length - 1].y_ratio * -1) + '}');
  }
  console.log(check);
}
///////////////////
